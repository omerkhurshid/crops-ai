            ...
            {loading ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <Satellite className="h-12 w-12 mx-auto mb-4 opacity-50 text-[#555555]" />
                  <h3 className="text-lg font-semibold mb-2 text-[#555555]">No Fields Available</h3>
                  <p className="text-[#555555]">Add fields to your farm to see crop health data.</p>
                </div>
              </div>
            ) : (
              <>
                    ...
                    {currentFieldData && (
                      <div className="h-full w-full relative">
                        ...
                        <div className="absolute inset-8 border-2 border-gray-600 bg-gradient-to-br from-green-300 via-green-400 to-green-500 rounded-lg overflow-hidden">
                          ...
                          {(currentFieldData.zones?.averageAreas?.percentage || 0) > 0 && (
                          ...
                          {(currentFieldData.zones?.thrivingAreas?.percentage || 0) > 0 && (
                    ...
                    <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-lg p-4 max-w-xs">
                      ...
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          <span className="text-[#555555]">Plant Health Score:</span>
                          <span className="font-medium">{(currentFieldData.healthAverage * 100).toFixed(0)}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#555555]">Overall Status:</span>
                          <span className={cn("font-medium", currentFieldData.healthAverage > 0.6 ? "text-[#8FBF7F]" : "text-yellow-600")}>
                            {getHealthLabel(currentFieldData.healthAverage)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#555555]">Trend:</span>
                          <span className={cn("font-medium flex items-center gap-1", currentFieldData.trend > 0 ? "text-[#8FBF7F]" : "text-red-600")}>
            ...
            <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge variant="destructive">{currentFieldData.zones?.problemAreas?.percentage || 0}%</Badge>
                </div>
                <p className="text-xs text-red-700">{(currentFieldData.zones?.problemAreas?.area || 0).toFixed(1)} ha</p>
              </div>
              <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-yellow-900">Average Areas</span>
                  <Badge className="bg-yellow-500 text-white">{currentFieldData.zones?.averageAreas?.percentage || 0}%</Badge>
                </div>
                <p className="text-xs text-yellow-700">{(currentFieldData.zones?.averageAreas?.area || 0).toFixed(1)} ha</p>
              </div>
              <div className="bg-[#F8FAF8] rounded-lg p-4 border border-[#DDE4D8]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-green-900">Healthy Areas</span>
                  <Badge className="bg-[#8FBF7F] text-white">{currentFieldData.zones?.healthyAreas?.percentage || 0}%</Badge>
                </div>
                <p className="text-xs text-[#7A8F78]">{(currentFieldData.zones?.healthyAreas?.area || 0).toFixed(1)} ha</p>
              </div>
              <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-emerald-900">Thriving Areas</span>
                  <Badge className="bg-emerald-500 text-white">{currentFieldData.zones?.thrivingAreas?.percentage || 0}%</Badge>
                </div>
                <p className="text-xs text-emerald-700">{(currentFieldData.zones?.thrivingAreas?.area || 0).toFixed(1)} ha</p>
              </div>
            </div>
          )}
        </CardContent>
              ...
              {currentFieldData ? (
                <>
                  {(currentFieldData.zones?.problemAreas?.percentage || 0) > 10 && (
                    <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                      ...
                      <p className="text-sm text-yellow-800">
                        {currentFieldData.zones?.problemAreas?.percentage || 0}% of the field ({(currentFieldData.zones?.problemAreas?.area || 0).toFixed(1)} ha) shows signs of stress. 
                  ...
                  <div className="p-4 bg-[#F8FAF8] rounded-lg border border-[#DDE4D8]">
                    ...
                    <p className="text-sm text-[#7A8F78]">
                      {((currentFieldData.zones?.healthyAreas?.percentage || 0) + (currentFieldData.zones?.thrivingAreas?.percentage || 0)).toFixed(0)}% 
                      of your field ({((currentFieldData.zones?.healthyAreas?.area || 0) + (currentFieldData.zones?.thrivingAreas?.area || 0)).toFixed(1)} ha) 
